...
def generateInterferenceProfiles(cnf):
    entry = input("Generate interference profiles? [y/N] ") or "n"
    if entry in ['Y','y']:  
...
def testInterference(cnf):
    entry = input("Test interference? [y/N] ") or "n"
    if entry in ['Y','y']:
        mconsole("Testing interference")
        runInterference(cnf)
