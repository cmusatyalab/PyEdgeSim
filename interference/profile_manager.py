...
def testcodegraveyard():
    aa.saveProfDict()
    allprof = aa.readProfDict()
    newallprof = aa.copyProf('zero',npf = "mynewzero")
    newblankprof = aa.newProf(name = "newblank",application = "edgevdi")
    newblankprof.composeProf(initcond = 0, exceptions = 2,netcharevents = 3)
    newblankprof.addInitialConditions(DEFINIT)
    
    chardict = {'newmover':'UE1','newdest': 'zone1-poa2','newwaitafter':5}
    newblankprof.modifyMobilityEvent(0,**chardict)
    newblankprof.modifyNetCharEvent(1, newlatency = 10,
                 newlatencyVariation=2,newthroughput=100, newpacketLoss=1,
                 newname="zone1-poa1",newwaitafter = 5)
    chardict = {'newlatency':20,'newlatencyVariation':4,'newthroughput':200, 'newpacketLoss':2,
                 'newname':"zone1-poa2",'newwaitafter':5}
    newblankprof.modifyNetCharEvent(2,**chardict)
    
    chardict = {'newlatency':20,'newlatencyVariation':4,'newthroughput':200, 'newpacketLoss':2,
             'newname':"zone1-poa2"}
    newblankprof.modifyException(0,**chardict)
    
    newblankprof.prettyPrint()
    newblankprof.saveProf("./{}.json".format('newblank'))
    aa.addProf(newblankprof.getProfData())
    aa.addProf(newblankprof.getProfData()) # Do twice to check replace
